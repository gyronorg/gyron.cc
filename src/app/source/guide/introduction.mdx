...
`Gyron`是一款灵活的 MVVM 框架，压缩后可减少到 6.9KB ，相比于其它框架，体积上面它有更大的优势。

`Gyron`的视图更新尽可能最小，用户可以在生命周期钩子中告诉`Gyron`，此次数据更新不需要自动渲染。
...
```bash
npm add @gyron/runtime @gyron/babel-plugin-jsx vite -D
```

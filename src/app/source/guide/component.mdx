...
有的时候我们需要自定义渲染，或者想获取到真实的元素信息。我们可以使用`useRef`方法创建一个对象，它会暴露一个`current`属性，然后在对应的组件或元素上使用 ref 进行绑定。
...
```jsx
import { keepComponent, useValue } from 'gyron'
...
```jsx line=7
import { clearCacheComponent, keepComponent } from 'gyron'

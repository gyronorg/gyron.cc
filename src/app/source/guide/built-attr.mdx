...
```jsx line=4,11
import { useRef, onAfterMount, exposeComponent } from 'gyron'
...
\_html 属性是为了解决服务端渲染时插入动态的子节点，在客户端渲染时也可以使用。

```jsx
const App = ({ orders }) => {
  return <div _html="<h1>Gyron</h1>"></div>
}
```

如果使用了\_html 属性的同时还使用了 children，那么现在以 children 属性为准，并且控制台将会报告此警告。

```jsx
const App = ({ orders }) => {
  return <div _html="<h1>Hello Gyron</h1>">Welcome Gyron</div>
}
